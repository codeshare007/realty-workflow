...
import { AddressDto, ContactDto, ContactInfoDto, ContactType, CreateSigningContactInput, SigningContactServiceProxy, UpdateSigningContactInput } from '@shared/service-proxies/service-proxies';
...
@Component({
    selector: 'signing-contacts',
    templateUrl: './signing-contacts.component.html',
    styleUrls: ['./signing-contacts.component.less'],
    encapsulation: ViewEncapsulation.None,
    animations: [accountModuleAnimation()]
})
export class SigningContactsComponent extends AppComponentBase implements OnInit, OnDestroy {
    ...
    constructor(
        injector: Injector,
        private _signingContactServiceProxy: SigningContactServiceProxy,
    ...
    getContacts(event?: LazyLoadEvent) {
        ...
        this._signingContactServiceProxy.getAll(
    ...
    public selectOption(element: { item: UiTableActionItem, id: string }): void {
        switch (element.item.name) {
            case this.l('Edit'):
                this._signingContactServiceProxy.getForEdit(element.id, this.signingId)
                    .subscribe(contact => {
                        this.createEditContactModal.show(contact);
                    });
                break;
            case this.l('Delete'):
                this._signingContactServiceProxy.delete(element.id, this.signingId)
                    .subscribe(contact => {
                        this.getContacts();
                    });
                break;
    ...
    public onContactCreate(contact: ContactDto) {
        if (!contact.id) {
            let input = new CreateSigningContactInput();
            input.signingId = this.signingId;
            input.contact = contact;

            this._signingContactServiceProxy.create(input)
            .subscribe(r => {
                this.getContacts();
            });
        } else {
            let input = new UpdateSigningContactInput();
            input.signingId = this.signingId;
            input.contact = contact;

            this._signingContactServiceProxy.update(input)
            .subscribe(r => {
                this.getContacts();
            });
        }
